...
async function startServer() {
  ...
  app.get("/api/emails", async (req, res) => {
    ...
    res.json({ success: true, emails, folder });
  });
...
// ─── Himalaya IMAP email fetcher ───
async function fetchEmailsFromHimalaya(limit = 50, folder = 'INBOX'): Promise<any[]> {
  try {
    const { stdout } = await execAsync(
      `himalaya envelope list -f ${folder} -s ${limit} --output json 2>/dev/null`,
      { timeout: 15000 }
    );
